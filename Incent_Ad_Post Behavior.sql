...
--------------------------------------------------------------------
SELECT segment, variant, usersystem, country, DATEDIFF(day, date_joined_test, date_of_agg) as days_since_joined_test, count(x.user_id) as players, 
...
FROM (  --Master list of participants
      ...
      MIN(sessiontime) as date_joined_test
